...
def register(request):
	...
	return render_to_response('register.html',{'User':u},
		context_instance=RequestContext(request)) #Needed for the template to access the static files. 
...
def add_submit(request):
	...
	if next == "stories":
		...
		return HttpResponseRedirect(reverse('stories.views.%s' %next, kwargs={'article_id':article_id}))
		# the reverse tells the program to go to views.next, tell it what url it is, then go back to this program with the URL.
		# If you skipped the reverse, it would go straight to the URL, which would make it go wonky if your URL changes.
...
def edit_submit(request, article_id):
	...
	a.notes = request.POST['notes']
	a.category = request.POST['category']
	b = request.POST['sources'].rsplit(',') #takes the string and returns a list of the results separated by the separator given -- here, the comma
	list = []
	for i in range (0,len(b)-1):
		if b[i] not in list:
			list.append(b[i])
...
def source_query(request): 
	query = request.GET['q']
	source_list = request.user.sources.filter( #python query to the database
